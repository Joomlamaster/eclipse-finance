			...
			$.each(tradeApp.rates, function(index, rate) {
				...
				if( d < 1 ) { //1 second 
					if(rate.processing !== true) {
						...
						$.get('rates/check', {rate_id: rate.rate_id,status: 3}, function(data) {
							if( data ) {
	...
	var updateOpenedRates = function() {
		...
		$.each(tradeApp.rates, function(rateId, rateObj) {
			...
			if(symbolList[rateObj.currency]) {
				...
				if( win ) {
					var uniqueclass = "unique"+rateId;
					rate.attr('class', 'win ' + uniqueclass); 
					var price = $('tr.win.'+uniqueclass+' td:nth-child(7)').text().replace('$', '');
					var actualPrice = '$ '+ Math.round(1.85*price);
					$('tr.win.'+uniqueclass+' td:nth-child(8)').text(actualPrice);
