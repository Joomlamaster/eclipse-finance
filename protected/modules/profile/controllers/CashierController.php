...
class CashierController extends ProfileController
{
	...
	{   
                ...
                $transactions = Transactions::model()->findAll();//BonusToUsers::model()->findAll();
                
                $deductions = DeductionToUsers::model()->findAll();
                
                $bonus = BonusToUsers::model()->findAll();
                
                $WithdrawalRequests = WithdrawalRequests::model()->findAll();
                
                echo 'Transactions'; echo '<br/>';
                foreach($transactions as $transaction){
                    echo date('Y-m-d h:i:s', $transaction->timestamp);
                    echo '<br/>';
                }
                echo 'Deductions'; echo '<br/>';
                foreach($deductions as $deduction){
                    echo date('Y-m-d h:i:s', $deduction->deduction_timestamp);
                    echo '<br/>';
                }
                echo 'Bonus'; echo '<br/>';
                foreach($bonus as $singleBonus){
                    echo date('Y-m-d h:i:s', $singleBonus->bonus_timestamp);
                    echo '<br/>';
                }
                
                echo 'WithdrawalRequests'; echo '<br/>';
                foreach($WithdrawalRequests as $WithdrawalRequest){
                    echo date('Y-m-d h:i:s', $WithdrawalRequest->timestamp);
                    echo '<br/>';
                }
	...
	function actionSendRequest()
	{
		...
		if(isset($_POST['DepositForm']))
		{
			...
			{   
				...
				$ApiPassword_encrypt= hash('sha256',$ApiPassword); //'d2418bc7bf1a83229917b7077bf427280bf416cf30145eba5b2fdc3d1a0213af'; 				          	    //<TransactionRequest xmlns="https://www.merchantsworldwide.net/acquiring.php">
 				//<TransactionRequest xmlns="https://www.lamdaprocessing.com/securePayments/direct/v1/processor.php">
				$xmlReq='<?xml version="1.0" encoding="UTF-8" ?>
				<TransactionRequest xmlns="https://merchantsworldwide.net/acquiring.php">
